    ...
    <>
      <GridView title={'Danh sách ' + titleTable} handleChange={handleChange} loading={isLoading}>
        ...
        >
          ...
          />
          <GridView.Table.Column style={{}} title="Tên đối tượng" sortKey="name" body={({ item }) => (<span>{item.name}</span>)} />
          <GridView.Table.Column style={{}} title="Mô tả" sortKey="description" body={({ item }) => (<span>{item.description}</span>)} />
          <GridView.Table.Column
            style={{ width: "10%" }}
            ...
            body={({ item }) => (
              <div className="flex flex-row">
                ...
                {permisson.per_Delete && <AiFillDelete
                  ...
                  title="Xóa"
                  onClick={() => delAction(item, rolesServices, data, setMeta, meta, mutate)}
                />
                }
              </div>
            )}
      ...
      <RolesForm show={state.show} onClose={onClose} action={state.action} id={state.Id} />
      <ConfirmationDialog />
    </>
  );
}
