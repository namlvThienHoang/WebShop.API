...
namespace EPS.Utils.Common
{
    public class TUtils
    {
        public static void SetStyleExcel(ref ExcelWorksheet sheet, List<ClsExcel> lstValue)
        {
            ...
            {
                sheet.Column(col).Width = sheet.Column(col).Width + 1;
            }
        ...
        /// <summary>
        /// get content type fileattachment
        /// </summary>
        ...
        public static string Encrypt(string toEncrypt, string keyMaHoa)
        {
            ...
            if (useHashing)
            {
                ...
                keyArray = hashmd5.ComputeHash(UTF8Encoding.UTF8.GetBytes(keyMaHoa));
            }
            else
                keyArray = UTF8Encoding.UTF8.GetBytes(keyMaHoa);
            ...
            string temp = BitConverter.ToString(resultArray).Replace("-", string.Empty);
            return temp;
        }
        ...
        public static string Decrypt(string toDecrypt, string keyMaHoa)
        {
            byte[] toEncryptArray = new byte[toDecrypt.Length / 2];
            for (int i = 0; i < toDecrypt.Length; i += 2)
                toEncryptArray[i / 2] = Convert.ToByte(toDecrypt.Substring(i, 2), 16);
